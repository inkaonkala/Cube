...
/*   Updated: 2024/11/06 12:33:42 by iniska           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ghostie(t_game *game)
{
	...
	x = (game->enemy->angl / (FOW / 2)) * (WINDOW_WIDTH / 2) + (WINDOW_WIDTH / 2);
	//size = TILE / game->enemy->distance;
	mlx_image_to_window(game->mlx, game->enemy->ghosty, x, game->enemy->g_y * TILE / game->enemy->distance);
}