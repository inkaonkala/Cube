...
/*   Updated: 2024/11/14 21:02:09 by iniska           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void update_enemy(t_game *game)
{
    ...
    enemy_hi = fabs(((TILE - 30) / game->enemy->distance) * (WINDOW_WIDTH) / tan(FOW));
	...
	*/
    ...
    screen_x = (int)(WINDOW_WIDTH / 2 + (game->enemy->angl * WINDOW_WIDTH / FOW)); // check player FOW
    //screen_y = (int)(top_pixl);
    ...
    for (int y = (int)top_pixl; y < (int)bot_pixl; y++) 
	{
        int src_y = (int)((y - top_pixl) / enemy_hi * game->enemy->height) + frame_y;
        
        if (src_y >= 0 && src_y < (int)pic->height) 
		{
            for (int x_offset = 0; x_offset < game->enemy->len; x_offset++) 
			{
